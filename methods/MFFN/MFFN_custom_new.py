...
class ASPP(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.conv3 = ConvBNReLU(in_dim, out_dim, kernel_size=3, dilation=2, padding=2) # experiment w/ dilation/padding
        self.conv4 = ConvBNReLU(in_dim, out_dim, kernel_size=3, dilation=2, padding=2) # experiment w/ dilation/padding
        self.conv5 = ConvBNReLU(in_dim, out_dim, kernel_size=3, dilation=5, padding=5)
        self.conv6 = ConvBNReLU(in_dim, out_dim, kernel_size=3, dilation=7, padding=7)
        self.conv7 = ConvBNReLU(in_dim, out_dim, kernel_size=1)
        ...
        self.fuse = ConvBNReLU(7 * out_dim, out_dim, 3, 1, 1) # NOTE: changed for # of features
    def forward(self, x):
        conv1 = self.conv1(x)
        conv2 = self.conv2(x)
        conv3 = self.conv3(x)
        ...
        conv5 = self.conv5(x)
        conv6 = self.conv6(x)
        conv7 = self.conv7(cus_sample(x.mean((2, 3), keepdim=True), mode="size", factors=x.size()[2:]))
        return self.fuse(torch.cat((conv1, conv2, conv3, conv4, conv5, conv6, conv7), 1))

class TransLayer(nn.Module):
    def __init__(self, out_c, last_module=ASPP):
        super().__init__()

        self.c7_down = nn.Sequential(
            # ConvBNReLU(2048, 256, 3, 1, 1),
            last_module(in_dim=2048, out_dim=out_c),
        )
        self.c6_down = nn.Sequential(ConvBNReLU(1024, out_c, 3, 1, 1))
        self.c5_down = nn.Sequential(ConvBNReLU(512, out_c, 3, 1, 1))
        self.c4_down = nn.Sequential(ConvBNReLU(256, out_c, 3, 1, 1))
    ...
    def forward(self, xs):
        ...
        assert len(xs) == 7
        c1, c2, c3, c4, c5, c6, c7 = xs  # Unpack c6 and c7

        c7 = self.c7_down(c7)  # Apply c7 layer
        c6 = self.c6_down(c6)  # Apply c6 layer
        c5 = self.c5_down(c5)
        c4 = self.c4_down(c4)
        c3 = self.c3_down(c3)
        c2 = self.c2_down(c2)
        c1 = self.c1_down(c1)
        return c7, c6, c5, c4, c3, c2, c1
        # assert len(xs) == 5
        # c1, c2, c3, c4, c5 = xs
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = self.fc2(self.relu1(self.fc1(self.avg_pool(x)))) # avg/max
        max_out = self.fc2(self.relu1(self.fc1(self.max_pool(x))))
        out = avg_out + max_out
        return self.sigmoid(out)
    
# attention across spatial?
...
class CAMV(nn.Module):
    def __init__(self, in_dim, mm_size):
        ...
        self.conv_l_pre_down = ConvBNReLU(in_dim, in_dim, 5, stride=1, padding=2) # NOTE: larger kernel size closer zoom?
        ...
        # for combining triplets of views
        ...
        # badly named variables but for processing angles
        ...
        # for processing contrast/brightness
        self.transcon1 = nn.Sequential(
            ConvBNReLU(in_dim, in_dim, 3, 1, 1),
            ConvBNReLU(in_dim, in_dim, 3, 1, 1),
        )
        self.transcon2 = nn.Sequential(
            ConvBNReLU(in_dim, in_dim, 3, 1, 1),
            ConvBNReLU(in_dim, in_dim, 3, 1, 1),
        )
        ...
        # CONTRAST/BRIGHTNESS VIEWS
        self.coe_c_con1 = nn.Parameter(data=torch.Tensor(1,64), requires_grad=True)
        self.coe_h_con1 = nn.Parameter(data=torch.Tensor(mm_size,mm_size), requires_grad=True)
        self.coe_w_con1 = nn.Parameter(data=torch.Tensor(mm_size,mm_size), requires_grad=True)
        
        self.coe_c_mcon = nn.Parameter(data=torch.Tensor(1,64), requires_grad=True)
        self.coe_h_mcon = nn.Parameter(data=torch.Tensor(mm_size,mm_size), requires_grad=True)
        self.coe_w_mcon = nn.Parameter(data=torch.Tensor(mm_size,mm_size), requires_grad=True)
        
        self.coe_c_con2 = nn.Parameter(data=torch.Tensor(1,64), requires_grad=True)
        self.coe_h_con2 = nn.Parameter(data=torch.Tensor(mm_size,mm_size), requires_grad=True)
        self.coe_w_con2 = nn.Parameter(data=torch.Tensor(mm_size,mm_size), requires_grad=True)
        
        self.coe_c_con1.data.uniform_(-0.5,0.5)
        self.coe_h_con1.data.uniform_(-0.5,0.5)
        self.coe_w_con1.data.uniform_(-0.5,0.5)
        
        self.coe_c_mcon.data.uniform_(-0.5,0.5)
        self.coe_h_mcon.data.uniform_(-0.5,0.5)
        self.coe_w_mcon.data.uniform_(-0.5,0.5)
        
        self.coe_c_con2.data.uniform_(-0.5,0.5)
        self.coe_h_con2.data.uniform_(-0.5,0.5)
        self.coe_w_con2.data.uniform_(-0.5,0.5)

        self.channel_attn = ChannelAttention(64)
        self.spatial_attn = SpatialAttention()

        # NOTE: fuse value changes
        self.fuse = nn.Sequential(ConvBNReLU(192, 128, 1),ConvBNReLU(128, 64, 3,1,1),ConvBNReLU(64, 64, 3,1,1))
    
    def forward(self, con1, con2, c1, o, c2, a1, a2, return_feats=False):
        tgt_size = o.shape[2:]

        # CLOSE VIEWS
        # on largeer zoom: conv, pool, then  conv
        c1 = self.conv_l_pre_down(c1) 
        c1 = F.adaptive_max_pool2d(c1, tgt_size) + F.adaptive_avg_pool2d(c1, tgt_size) # adaptive pooling to output tgt_size
        c1 = self.conv_l_post_down(c1)

        m = self.conv_m(o)

        # on smaller zoom: conv, pool, then  conv
        c2 = self.conv_s_pre_up(c2)
        c2 = F.adaptive_max_pool2d(c2, tgt_size) + F.adaptive_avg_pool2d(c2, tgt_size)
        c2 = self.conv_s_post_up(c2)
        attn1 = self.trans1(torch.cat([c1, m, c2], dim=1)) # NOTE: Trans layer

        # NOTE: more efficient dot products w/ tensorly?
        attn_c1 = tl.tenalg.mode_dot(attn1,self.coe_c_c1,mode=1)
        ...
        attn_md = tl.tenalg.mode_dot(attn1,self.coe_c_md,mode=1)
        ...
        attn_c2 = tl.tenalg.mode_dot(attn1,self.coe_c_c2,mode=1)
        ...
        a1 = self.transa1(a1)
        a2 = self.transa2(a2)
        attn2 = self.trans2(torch.cat([a1, m, a2], dim=1))
        
        attn_a1 = tl.tenalg.mode_dot(attn2,self.coe_c_a1,mode=1)
        ...
        attn_ma = tl.tenalg.mode_dot(attn2,self.coe_c_ma,mode=1)
        ...
        attn_a2 = tl.tenalg.mode_dot(attn2,self.coe_c_a2,mode=1)
        ...
        # CONTRAST/BRIGHTNESS VIEWS
        con1 = self.transa1(con1)
        con2 = self.transa2(con2)
        attn3 = self.trans2(torch.cat([con1, m, con2], dim=1))
        
        attn_con1 = tl.tenalg.mode_dot(attn3,self.coe_c_con1,mode=1)
        attn_con1 = tl.tenalg.mode_dot(attn_con1,self.coe_h_con1,mode=2)
        attn_con1 = tl.tenalg.mode_dot(attn_con1,self.coe_w_con1,mode=3)
        attn_con1 = torch.softmax(attn_con1, dim=1)
        
        attn_mcon = tl.tenalg.mode_dot(attn3,self.coe_c_mcon,mode=1)
        attn_mcon = tl.tenalg.mode_dot(attn_mcon,self.coe_h_mcon,mode=2)
        attn_mcon = tl.tenalg.mode_dot(attn_mcon,self.coe_w_mcon,mode=3)
        attn_mcon = torch.softmax(attn_mcon, dim=1)
        
        attn_con2 = tl.tenalg.mode_dot(attn3,self.coe_c_con2,mode=1)
        attn_con2 = tl.tenalg.mode_dot(attn_con2,self.coe_h_con2,mode=2)
        attn_con2 = tl.tenalg.mode_dot(attn_con2,self.coe_w_con2,mode=3)
        attn_con2 = torch.softmax(attn_con2, dim=1)
        
        # supposedly, dont need channel/space attn for contrast/brightness bc not rotated
        # read paper for more
        cbmcb = attn_con1 * con1 + attn_mcon * m + attn_con2 * con2

        # concatenation of (angle, unmodified, angle) + (close, unmodified, close)
        lms = self.fuse(torch.cat([ama,cmc, cbmcb],dim=1))
...
class CFU(nn.Module):
    def __init__(self, in_c, num_groups=4, hidden_dim=None):
        ...
        self.fp = Progressive_Iteration(192) # NOTE: change to 384?
...
# NOTE: main MFFN
@MODELS.register()
class MFFN_custom_new(BasicModelClass):
    def __init__(self):
        super().__init__()
        self.shared_encoder = timm.create_model(model_name="resnet50", pretrained=True, in_chans=3, features_only=True) # feature pyramid encoder
        self.translayer = TransLayer(out_c=64)  # [c5, c4, c3, c2, c1] view combining
        # dim = [64, 64, 64, 64, 64]
        # size = [12, 24, 48, 96, 192] # upsampling
        dim = [64, 64, 64, 64, 64, 64, 64]
        size = [12, 24, 48, 96, 192, 384, 768] 
        self.CAMV_layers = nn.ModuleList([CAMV(in_dim=in_c, mm_size=mm_s) for in_c, mm_s in zip(dim, size)])

        # NOTE: CFUs according to # of views. reduced dim bc test shows not necessary
        self.d7 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d6 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d6 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d5 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d4 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d3 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d2 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
        self.d1 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=16))
    ...
    # NOTE: upsampling of CFUs
    def body(self, con1_scale, con2_scale, c1_scale, o_scale, c2_scale, a1_scale, a2_scale):
        con1_trans_feats = self.encoder_translayer(con1_scale)
        con2_trans_feats = self.encoder_translayer(con2_scale)
        ...
        feats = []
        for con1,con2,c1,o,c2,a1,a2, layer in zip(con1_trans_feats, con2_trans_feats, c1_trans_feats, o_trans_feats, c2_trans_feats, a1_trans_feats, a2_trans_feats, self.CAMV_layers):
            CAMV_outs = layer(con1=con1, con2=con2, c1=c1, o=o, c2=c2, a1=a1, a2=a2)
            feats.append(CAMV_outs)


        # NOTE: NEW for extra views
        x = self.d7(feats[0])
        ...
        # Not used supposedly due to not complete ASSP
        x = self.d2(x + feats[5])
        x = cus_sample(x, mode="scale", factors=2)
        x = self.d1(x + feats[6])
        x = cus_sample(x, mode="scale", factors=2)
    ...
    def train_forward(self, data, **kwargs):
        assert not {"image_con1", "image_con2", "image_c1", "image_o", "image_c2", "image_a1", "image_a2", "mask"}.difference(set(data)), set(data)
        output = self.body(
            con1_scale=data["image_con1"],
            con2_scale=data["image_con2"],
