...
class CAMV(nn.Module):
    def __init__(self, in_dim, mm_size):
        ...
        self.channel_attn1 = ChannelAttention(64)
        self.spatial_attn = SpatialAttention()

        self.channel_attn2 = ChannelAttention(64)
        self.color_attn = SpatialAttention()
    ...
    def forward(self, con1, con2, c1, o, c2, a1, a2, return_feats=False):
        ...
        ama = ama.mul(self.channel_attn1(ama))
        ama = ama.mul(self.spatial_attn(ama))

        # CONTRAST/BRIGHTNESS VIEWS
        con1 = self.transcon1(con1)
        con2 = self.transcon2(con2)
        attn3 = self.trans3(torch.cat([con1, m, con2], dim=1))
        ...
        # "color attention" idea didnt work well
        cbmcb = cbmcb.mul(self.channel_attn2(cbmcb))
        cbmcb = cbmcb.mul(self.color_attn(cbmcb))
...
def cal_ual(seg_logits, seg_gts):
    ...
    return loss_map.mean()
        ...
        # NOTE: main MFFN
@MODELS.register()
class MFFN_custom(BasicModelClass):
    def __init__(self):
        super().__init__()
        ...
        # NOTE: CFUs according to # of views
        self.d7 = nn.Sequential(CFU(64, num_groups=6, hidden_dim=32))
    ...
    def body(self, con1_scale, con2_scale, c1_scale, o_scale, c2_scale, a1_scale, a2_scale):
        ...
        # NOTE: mapping each features together for all parts -- COOL! don't need extra downsampling
