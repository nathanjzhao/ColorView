...
class MFFN_COD_TestDataset(_BaseSODDataset):
    def __init__(self, root: Tuple[str, dict], shape: Dict[str, int], interp_cfg: Dict = None):
        ...
        # self.image_norm = A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)) # imagenet
        self.image_norm = A.Normalize(mean=(0.381809, 0.455771, 0.46259365387), std=(0.28151, 0.258233, 0.259145)) # acd1k + camo
        # self.image_norm = A.Normalize(mean=(0.339838, 0.43831709559, 0.45298364551 ), std=(0.262845, 0.250552, 0.25906)) # acd1k
...
class MFFN_COD_TrainDataset(_BaseSODDataset):
    def __init__(
        ...
        self.joint_trans = A.Compose(
            [
                ...
                A.Normalize(mean=(0.381809, 0.455771, 0.46259365387), std=(0.28151, 0.258233, 0.259145)) # acd1k + camo
                # A.Normalize(mean=(0.339838, 0.43831709559, 0.45298364551 ), std=(0.262845, 0.250552, 0.25906)), # acd1k
            ],
        )
    ...
    def __getitem__(self, index):
        ...
        transformed0 = self.joint_trans(image=image0, mask=mask) # NOTE: only probabilistic transformations?
